...
<div class="pagamentos view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Forma') ?></th>
            <td><?= h($pagamento->forma) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Loja') ?></th>
            <td><?= $pagamento->has('loja') ? $this->Html->link($pagamento->loja->id, ['controller' => 'Lojas', 'action' => 'view', $pagamento->loja->id]) : '' ?></td>
        </tr>
        <tr>
